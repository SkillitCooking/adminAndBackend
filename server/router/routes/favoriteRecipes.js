    ...
    User.model.findById(req.body.userId, 'curToken', function(err, user) {
      if(user) {
        ...
        FavoriteRecipe.model.aggregate([{$match: {userId: mongoose.Types.ObjectId(req.body.userId)}}, 
          {$project: {timesUsed: {$size: '$datesUsed'}, recipeIds: 1, ingredientAndFormIds: 1, ingredientNames: 1, userId: 1, dateLastUsed: 1, lastSeasoningProfileUsed: 1, name: 1, description: 1, mainPictureURL: 1, prepTime: 1, totalTime: 1}}], function(err, favRecipes) {
          if(err) {
